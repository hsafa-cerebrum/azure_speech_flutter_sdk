...
public class SwiftAzureSpeechRecognitionPlugin: NSObject, FlutterPlugin {
    ...
    // Subscription key and region saved here for reuse
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "startTranscription":
            guard let key = args?["subscriptionKey"] as? String,
                  let region = args?["region"] as? String else {
                result(FlutterError(code: "400", message: "Missing subscriptionKey or region", details: nil))
                return
            }
            subscriptionKey = key
    ...
    private func startTranscription() {
        do {
            ...
            transcriber!.addTranscribedEventHandler() { [weak self] _, evt in
                ...
                print("Received transcription: \(text), speakerId: \(speakerId)")
                self.channel?.invokeMethod("speech.onTranscribedWithSpeaker", arguments: [
                    "text": text,
                    "speakerId": speakerId
                ])
            }
            ...
            transcriber!.addSessionStartedEventHandler() { _, _ in
                ...
                self.channel?.invokeMethod("speech.onSessionStarted", arguments: nil)
            }

            transcriber!.addSessionStoppedEventHandler() { [weak self] _, _ in
                print("Session stopped")
                self?.operationCompleted = true
                self?.channel?.invokeMethod("speech.onSessionStopped", arguments: nil)
            }

            transcriber!.addCanceledEventHandler() { [weak self] _, evt in
                print("Session canceled: \(evt.errorDetails ?? "unknown error")")
                self?.channel?.invokeMethod("speech.onSessionCanceled", arguments: evt.errorDetails)
            }

            operationCompleted = false
            try transcriber?.startTranscribingAsync({ started, error in
                if started {
                    print("Started transcription successfully")
    ...
    private func stopTranscription() {
        ...
        do {
            try transcriber.stopTranscribingAsync({ stopped, error in
                if stopped {
                    print("Stopped transcription successfully")
                } else if let error = error {
                    print("Failed to stop transcription: \(error.localizedDescription)")
                }
            })
        } catch {
            print("Error stopping transcription: \(error)")
        }
    }
